...
<div class="theory-anim-box">
  ...
  <div style="display:grid;grid-template-columns:repeat(2,1fr);gap:0.8rem;margin:1rem 0">
    <div class="t0-fadein" style="animation-delay:0.2s;background:rgba(108,92,231,0.12);border:1px solid rgba(108,92,231,0.25);border-radius:12px;padding:1rem">
      <div style="font-size:1.5rem;margin-bottom:0.3rem">⚡</div>
      <div style="font-weight:800;color:#6c5ce7;font-size:0.85rem">进程管理</div>
...
function animateSyscall(){var log=document.getElementById('syscallLog');var arrow=document.getElementById('syscallArrow');log.innerHTML='';var steps=['[用户态] print("Hello") 被调用','[用户态] 触发 sys_write 系统调用','[切换] CPU权限提升：用户态 → 内核态','[内核态] 执行 sys_write 处理函数','[内核态] 数据写入显示缓冲区','[切换] CPU权限降低：内核态 → 用户态','[用户态] 系统调用返回，继续执行'];var i=0;var intv=setInterval(function(){if(i>=steps.length){clearInterval(intv);arrow.style.color='rgba(0,206,201,0.8)';return;}log.innerHTML+='<div style="color:'+(steps[i].indexOf('切换')>=0?'#fd79a8':'rgba(255,255,255,0.6)')+'">→ '+steps[i]+'</div>';arrow.style.color=steps[i].indexOf('内核')>=0?'#fd79a8':'#00cec9';log.scrollTop=log.scrollHeight;i++;},600);}
    ...
    },
    {
      ...
      steps: [
        {
          title: "中断：硬件呼叫CPU的方式",
          content: '<p>当键盘被按下、网卡收到数据、磁盘读完文件时，硬件会通过<strong>中断</strong>通知CPU："我有事找你！"</p><div class="theory-anim-box"><div class="theory-anim-title">⚡ 中断处理流程</div><div id="interruptDemo" style="padding:1rem"><div style="display:flex;gap:0.8rem;align-items:center;justify-content:center;flex-wrap:wrap;margin-bottom:1rem"><div style="display:flex;flex-direction:column;gap:0.4rem;align-items:center"><div id="irqKbd" class="t0-fadein" style="animation-delay:0.2s;width:55px;height:55px;border-radius:50%;background:rgba(108,92,231,0.15);border:2px solid rgba(108,92,231,0.3);display:flex;align-items:center;justify-content:center;font-size:1.3rem;transition:all 0.3s;cursor:pointer" onclick="triggerIRQ(1)">⌨️</div><div style="font-size:0.6rem;color:rgba(255,255,255,0.4)">键盘</div></div><div style="display:flex;flex-direction:column;gap:0.4rem;align-items:center"><div id="irqDisk" class="t0-fadein" style="animation-delay:0.3s;width:55px;height:55px;border-radius:50%;background:rgba(0,206,201,0.15);border:2px solid rgba(0,206,201,0.3);display:flex;align-items:center;justify-content:center;font-size:1.3rem;transition:all 0.3s;cursor:pointer" onclick="triggerIRQ(6)">💾</div><div style="font-size:0.6rem;color:rgba(255,255,255,0.4)">磁盘</div></div><div style="display:flex;flex-direction:column;gap:0.4rem;align-items:center"><div id="irqNet" class="t0-fadein" style="animation-delay:0.4s;width:55px;height:55px;border-radius:50%;background:rgba(255,152,0,0.15);border:2px solid rgba(255,152,0,0.3);display:flex;align-items:center;justify-content:center;font-size:1.3rem;transition:all 0.3s;cursor:pointer" onclick="triggerIRQ(8)">🌐</div><div style="font-size:0.6rem;color:rgba(255,255,255,0.4)">网卡</div></div><div style="display:flex;flex-direction:column;gap:0.4rem;align-items:center"><div id="irqTimer" class="t0-fadein" style="animation-delay:0.5s;width:55px;height:55px;border-radius:50%;background:rgba(253,121,168,0.15);border:2px solid rgba(253,121,168,0.3);display:flex;align-items:center;justify-content:center;font-size:1.3rem;transition:all 0.3s;cursor:pointer" onclick="triggerIRQ(0)">⏱️</div><div style="font-size:0.6rem;color:rgba(255,255,255,0.4)">时钟</div></div></div><div style="text-align:center;margin:0.5rem 0"><div style="font-size:0.7rem;color:rgba(255,255,255,0.3)">⬇️ 中断请求（IRQ）⬇️</div></div><div style="background:rgba(253,121,168,0.1);border:2px solid rgba(253,121,168,0.25);border-radius:12px;padding:0.6rem;text-align:center"><div style="font-weight:800;color:#fd79a8;font-size:0.85rem">🧠 CPU — 中断处理</div><div id="irqStatus" style="font-size:0.7rem;color:rgba(255,255,255,0.5);margin-top:0.3rem">点击上方设备触发中断</div></div><div id="irqLog" style="margin-top:0.8rem;font-family:monospace;font-size:0.68rem;color:rgba(255,255,255,0.5);background:rgba(0,0,0,0.2);border-radius:8px;padding:0.5rem;max-height:80px;overflow-y:auto"></div></div></div><script>var irqNames={0:"时钟中断 → 更新时间，调度进程",1:"键盘中断 → 读取按键码",6:"磁盘中断 → I/O完成",8:"网卡中断 → 数据到达"};function triggerIRQ(n){var st=document.getElementById("irqStatus");var log=document.getElementById("irqLog");st.textContent="处理 IRQ"+n+": "+(irqNames[n]||"未知");st.style.color="#fd79a8";log.innerHTML+="<div>⚡ IRQ"+n+" → "+(irqNames[n]||"?")+"</div>";log.scrollTop=log.scrollHeight;setTimeout(function(){st.style.color="rgba(255,255,255,0.5)";},800);}</script>'
        },
        {
          title: "设备驱动：硬件的翻译官",
          content: '<p><strong>设备驱动程序</strong>是内核中与特定硬件通信的代码模块。没有驱动，内核就不知道怎么和硬件"说话"。</p><div class="theory-anim-box"><div class="theory-anim-title">🔌 设备驱动架构</div><div style="display:flex;flex-direction:column;gap:0.5rem;margin:1rem 0"><div class="t0-fadein" style="animation-delay:0.2s;background:rgba(108,92,231,0.1);border:1px solid rgba(108,92,231,0.25);border-radius:10px;padding:0.7rem;text-align:center"><div style="font-weight:800;color:#6c5ce7;font-size:0.8rem">📱 应用程序</div><div style="font-size:0.65rem;color:rgba(255,255,255,0.4)">print("Hello") / 打开文件 / 播放音乐</div></div><div style="text-align:center;color:rgba(255,255,255,0.2);font-size:0.7rem">⬇ 统一的系统调用接口</div><div class="t0-fadein" style="animation-delay:0.4s;background:rgba(0,206,201,0.1);border:1px solid rgba(0,206,201,0.25);border-radius:10px;padding:0.7rem;text-align:center"><div style="font-weight:800;color:#00cec9;font-size:0.8rem">🧠 内核核心</div><div style="font-size:0.65rem;color:rgba(255,255,255,0.4)">将请求分发给对应的驱动</div></div><div style="text-align:center;color:rgba(255,255,255,0.2);font-size:0.7rem">⬇ 驱动接口</div><div style="display:grid;grid-template-columns:repeat(4,1fr);gap:0.4rem" class="t0-fadein" style="animation-delay:0.6s"><div style="background:rgba(255,152,0,0.1);border:1px solid rgba(255,152,0,0.25);border-radius:8px;padding:0.5rem;text-align:center"><div style="font-size:1rem">🖥️</div><div style="font-size:0.6rem;color:#FF9800;font-weight:700">显卡驱动</div></div><div style="background:rgba(253,121,168,0.1);border:1px solid rgba(253,121,168,0.25);border-radius:8px;padding:0.5rem;text-align:center"><div style="font-size:1rem">🔊</div><div style="font-size:0.6rem;color:#fd79a8;font-weight:700">声卡驱动</div></div><div style="background:rgba(105,240,174,0.1);border:1px solid rgba(105,240,174,0.25);border-radius:8px;padding:0.5rem;text-align:center"><div style="font-size:1rem">💾</div><div style="font-size:0.6rem;color:#69F0AE;font-weight:700">磁盘驱动</div></div><div style="background:rgba(108,92,231,0.1);border:1px solid rgba(108,92,231,0.25);border-radius:8px;padding:0.5rem;text-align:center"><div style="font-size:1rem">🌐</div><div style="font-size:0.6rem;color:#6c5ce7;font-weight:700">网卡驱动</div></div></div><div style="text-align:center;color:rgba(255,255,255,0.2);font-size:0.7rem;margin-top:0.3rem">⬇ 硬件操作</div><div class="t0-fadein" style="animation-delay:0.8s;background:rgba(255,152,0,0.08);border:1px solid rgba(255,152,0,0.2);border-radius:10px;padding:0.5rem;text-align:center;font-size:0.7rem;color:rgba(255,255,255,0.5)">⚡ 实际硬件设备</div></div><div class="theory-callout">💡 驱动让内核能以<strong>统一的方式</strong>操作不同厂商的硬件。Linux中"一切皆文件"——连硬件设备也是通过文件接口访问！</div></div>'
        }
      ],
      challenge: {
