...
<div class="theory-anim-box">
  ...
  <div style="display:flex;flex-direction:column;gap:0.5rem;margin:1rem 0">
    ...
    <div class="t0-fadein" style="animation-delay:0.5s;background:linear-gradient(135deg,rgba(253,121,168,0.2),rgba(253,121,168,0.1));border:1px solid rgba(253,121,168,0.3);border-radius:12px;padding:0.8rem;text-align:center">
      <div style="font-weight:800;color:#fd79a8">🧠 操作系统内核</div>
...
<div class="theory-anim-box">
  ...
  <div id="cpuSchedulerDemo" style="padding:1rem">
    <div style="display:flex;gap:0.8rem;justify-content:center;flex-wrap:wrap;margin-bottom:1rem" id="schedTasks">
      ...
      <div class="sched-task" data-idx="2" style="padding:0.6rem 1rem;border-radius:10px;border:2px solid rgba(253,121,168,0.3);background:rgba(253,121,168,0.1);font-size:0.8rem;font-weight:700;transition:all 0.3s">📝 编辑器</div>
    ...
    <div style="text-align:center">
      <button onclick="startCpuScheduler()" style="padding:0.5rem 1.5rem;border-radius:8px;border:none;background:linear-gradient(135deg,#fd79a8,#ffb3d1);color:white;font-weight:700;cursor:pointer;font-size:0.8rem">▶ 开始调度</button>
    </div>
...
<script>
function startCpuScheduler(){var tasks=['浏览器','音乐','编辑器','终端'];var els=document.querySelectorAll('.sched-task');var log=document.getElementById('schedLog');log.innerHTML='';var colors=['rgba(253,121,168,0.4)','rgba(0,206,201,0.4)','rgba(253,121,168,0.4)','rgba(255,152,0,0.4)'];var i=0;var intv=setInterval(function(){els.forEach(function(e){e.style.boxShadow='none';e.style.transform='scale(1)';});if(i>=12){clearInterval(intv);log.innerHTML+='<div style="color:#69F0AE">✅ 调度完成！每个任务都得到了公平的CPU时间</div>';return;}var idx=i%4;els[idx].style.boxShadow='0 0 15px '+colors[idx];els[idx].style.transform='scale(1.1)';var ms=Math.floor(Math.random()*15)+5;log.innerHTML+='<div>[轮次'+(Math.floor(i/4)+1)+'] '+tasks[idx]+' → 获得 '+ms+'ms CPU时间</div>';log.scrollTop=log.scrollHeight;i++;},500);}
</script>`
        }
      ],
      challenge: {
      ...
      steps: [
        {
          ...
          content: '<p>每个程序都认为自己拥有一块连续的、巨大的内存空间——这就是<strong>虚拟内存</strong>的魔法。OS在背后默默做地址翻译。</p><div class="theory-anim-box"><div class="theory-anim-title">🗺️ 虚拟地址 → 物理地址映射</div><div style="display:flex;gap:1rem;margin:1rem 0;flex-wrap:wrap;justify-content:center"><div class="t0-fadein" style="animation-delay:0.2s;flex:1;min-width:150px"><div style="font-weight:800;color:#fd79a8;font-size:0.8rem;text-align:center;margin-bottom:0.5rem">🅰️ 程序A的视角</div><div style="display:flex;flex-direction:column;gap:0.2rem">' +
          '<div style="background:rgba(253,121,168,0.15);border:1px solid rgba(253,121,168,0.25);border-radius:4px;padding:0.3rem 0.5rem;font-size:0.65rem;font-family:monospace;display:flex;justify-content:space-between"><span style="color:rgba(255,255,255,0.4)">0x0000</span><span style="color:#fd79a8">代码段</span></div>' +
          ...
          '<div style="background:rgba(253,121,168,0.15);border:1px solid rgba(253,121,168,0.25);border-radius:4px;padding:0.3rem 0.5rem;font-size:0.65rem;color:#fd79a8">程序A的代码</div>' +
